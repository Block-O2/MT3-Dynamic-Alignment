...
![方法对比](simulation/results/method_comparison_bar.png)
![鲁棒性分析](simulation/results/robustness_analysis.png)

在干净条件下，三种方法的成功率都超过 97%。
Kalman 的稳态 RMS 误差最低（20.4mm，相比纯反应式控制的 21.4mm）。

在更接近真实传感器约束的条件下，Kalman 展现出明确的鲁棒性优势：
延迟抖动和帧丢失对三种方法的影响都较小，但在存在离群观测时，
**速度前馈方法显著退化（31.1mm，+50%）**；
Kalman 则通过协方差门控自动拒绝离群点，仍保持稳定表现（21.6mm）。

*注：Kalman 与纯反应式控制之间的差异预计会在真实硬件上更加明显，
因为真实机械臂存在电机惯性和控制带宽限制。纯反应式控制在离群点下也表现出一定鲁棒性，
原因是它不累积速度估计：单个位置尖峰只影响一帧，而不会通过速度模型继续传播。
但这种优势不会延伸到真实硬件场景；在真实机械臂惯性下，纯反应式控制会系统性滞后于移动物体。*
